...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'expiredProduct' | 'createdAt'> & {
  expiredProduct?: string | null;
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      expiredProduct: product.expiredProduct?.format(DATE_FORMAT) ?? null,
      createdAt: product.createdAt?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      expiredProduct: restProduct.expiredProduct ? dayjs(restProduct.expiredProduct) : undefined,
